...
export default {
  ...
  data () {
    return { email: '', emailErrors: '', password: '', passwordError: '', check: false, formErrors: [] }
  },
  methods: {
    LoginUser (e) {
      e.preventDefault()
      ...
      axios
        .post(url + 'login', userCredientals, {
          headers: {
        ...
        .catch((err) => {
          if (err.response.data.errors.email) {
            this.emailErrors = err.response.data.errors.email[0]
          } else {
            this.emailErrors = ''
          }
